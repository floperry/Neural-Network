    ...
    def _build_net(self):
        ...
        with tf.variable_scope(name) as scope:
            ...
            for i, node in enumerate(nodes[:-2]):
                layer = self._dense_layer(shape=nodes[i, i+2], inputs=layer, 
            ...
            for i, node in enumerate(nodes[::-1][:-2]):
                layer = self._dense_layer(shape=nodes[::-1][i, i+2], inputs=layer,
                                          layer_name='decoder_' + str(i+1))
                layer = tf.nn.relu(layer)

            self.outputs = self._dense_layer(shape=nodes[::-1][len(nodes)-2:len(nodes)],
                                             inputs=layer, layer_name='decoder_' + str(len(node)-1))
